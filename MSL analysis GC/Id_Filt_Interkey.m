...
pos_puntos45(pos_puntos45==0)=NaN;
                        ...
                        %% ACORDATE DE DESCOMENTAR ESTO PARA EL ANALISIS DE LOS SUJETOS
...
clf %clear figure
 ...
 %% Normalizacion zscore por bloque
% if flag_norm==1
%     
...
%     for i=1:noBlock
%        
%         media12=nanmean(filt12_block(i,:));
%         media23=nanmean(filt23_block(i,:));
%         media34=nanmean(filt34_block(i,:));
%         media45=nanmean(filt45_block(i,:));
%         
%         std12=nanstd(filt12_block(i,:));
%         std23=nanstd(filt23_block(i,:));
%         std34=nanstd(filt34_block(i,:));
%         std45=nanstd(filt45_block(i,:));
%         
%         %no uso la funcion zscore porque no contempla NaN's
%         filt12_block(i,:)=(filt12_block(i,:)-media12)/std12;
%         filt23_block(i,:)=(filt23_block(i,:)-media23)/std23;
%         filt34_block(i,:)=(filt34_block(i,:)-media34)/std34;
%         filt45_block(i,:)=(filt45_block(i,:)-media45)/std45;  
%         
%     end
% 
%     %vectorizo de nuevo para graficar
%     filt12_block=reshape(filt12_block',1, []);
%     filt23_block=reshape(filt23_block',1, []);
%     filt34_block=reshape(filt34_block',1, []);
%     filt45_block=reshape(filt45_block',1, []);
%     
%     %plot
%     figure;
%     plot(filt12_block,'b.','MarkerSize',15); hold on;
